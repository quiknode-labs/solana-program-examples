...
mod quasar_transfer_hook_switch {
    ...
    pub fn initialize_extra_account_metas_list(
        ...
        ctx.accounts.initialize_extra_account_metas_list()
    }
...
impl ConfigureAdmin {
    #[inline(always)]
    pub fn configure_admin(&mut self) -> Result<(), ProgramError> {
        let view = self.admin_config.to_account_view();
        let data = view.try_borrow()?;

        // If already initialised, verify caller is the current admin
        if data.len() >= 33 && data[32] != 0 {
            let admin_address = self.admin.to_account_view().address();
        ...
        if self.admin_config.to_account_view().address() != &admin_config_pda {
            return Err(ProgramError::InvalidSeeds);
        }

        // If account doesn't exist, create it
        if self.admin_config.to_account_view().data_len() == 0 {
            ...
            self.system_program
                .create_account(&self.admin, &*self.admin_config, lamports, size, &crate::ID)
                .invoke_signed(&seeds)?;
        }

        // Write new admin
        let mview = unsafe {
            &mut *(self.admin_config as *const UncheckedAccount as *mut UncheckedAccount
                as *mut AccountView)
        };
        let mut data = mview.try_borrow_mut()?;
        let new_admin_address = self.new_admin.to_account_view().address();
...
impl InitializeExtraAccountMetas {
    #[inline(always)]
    pub fn initialize_extra_account_metas_list(&mut self) -> Result<(), ProgramError> {
        ...
        let mint_address = self.token_mint.to_account_view().address();
        let (expected_pda, bump) = Address::find_program_address(
            &[b"extra-account-metas", mint_address.as_ref()],
            &crate::ID,
        );
        if self.extra_account_metas_list.to_account_view().address() != &expected_pda {
            return Err(ProgramError::InvalidSeeds);
        }
        ...
        self.system_program
            .create_account(&self.payer, &*self.extra_account_metas_list, lamports, meta_list_size, &crate::ID)
            .invoke_signed(&seeds)?;

        let view = unsafe {
            &mut *(self.extra_account_metas_list as *const UncheckedAccount
...
impl Switch {
    #[inline(always)]
    pub fn switch(&mut self, on: bool) -> Result<(), ProgramError> {
        // Verify admin
        let config_view = self.admin_config.to_account_view();
        ...
        let wallet_address = self.wallet.to_account_view().address();
        let (switch_pda, switch_bump) =
            Address::find_program_address(&[wallet_address.as_ref()], &crate::ID);
        if self.wallet_switch.to_account_view().address() != &switch_pda {
            return Err(ProgramError::InvalidSeeds);
        }

        if self.wallet_switch.to_account_view().data_len() == 0 {
            ...
            self.system_program
                .create_account(&self.admin, &*self.wallet_switch, lamports, size, &crate::ID)
                .invoke_signed(&switch_seeds)?;
        }

        let mview = unsafe {
            &mut *(self.wallet_switch as *const UncheckedAccount as *mut UncheckedAccount
                as *mut AccountView)
        };
        let mut data = mview.try_borrow_mut()?;
...
impl TransferHook {
    ...
    pub fn transfer_hook(&mut self) -> Result<(), ProgramError> {
        let switch_view = self.wallet_switch.to_account_view();
